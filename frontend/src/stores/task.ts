...
export const useTaskStore = defineStore(
	...
	() => {
		...
		/** 记录上一次 WS 是否已连接（用于\"断线→重连成功\"上升沿触发补拉历史） */
		...
		function connectWebSocket(taskId: string) {
			...
			ws = new TaskWebSocket(
				...
				(status) => {
					...
					// 连接成功（含 WS 断线后的重连成功）时补拉历史消息：
					// 断连期间后端 publish_message 仍会落盘到 logs/messages/{task_id}.json，
					// 仅靠 WS 实时推送会漏掉这段。用闭包记录上一状态，只在
					// "前状态非 connected → 当前 connected" 的上升沿触发，避免每次新建连接都重复拉取。
					if (status === "connected" && !wasWsConnected) {
						wasWsConnected = true;
						void loadTaskMessages(taskId);
					} else if (status !== "connected") {
						wasWsConnected = false;
					}
				},
			);
			ws.connect();
		...
		/**
		 * 加载任务的历史消息（供初始加载与 WS 重连成功时补拉）。
		 *
		 * 断连期间后端 publish_message 仍会落盘到 logs/messages/{task_id}.json，
		 * WS 重连后若不补拉，断连期间的消息就永久缺失（表现为"建模手册不显示""对话页停更"）。
		 * 这里重新调 getTaskMessages 并 mergeMessages，把后端已落盘但前端漏掉的消息补齐、去重。
		 */
	...
	},
	{
		persist: {
			key: "task-store",
			// 只持久化消息桶与当前任务 ID：这些是"刷新后应保留"的状态。
			// wsStatus / isRunning 依赖 WS 实时连接，刷新后从新连接重新判定，不持久化以免陈旧。
			pick: ["messagesByTask", "currentTaskId"],
		},
	},
);
